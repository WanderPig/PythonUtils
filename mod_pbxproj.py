...
class PBXBuildFile(PBXType):
    ...
    def add_attributes_flag(self, flag):
        k_settings = 'settings'
        k_attributes = 'ATTRIBUTES'
        ...
        if not s:
            self[k_settings] = PBXDict({k_attributes: PBXList([flag])})
            return True
...
class PBXCopyFilesBuildPhase(PBXBuildPhase):
    @classmethod
    def Create(cls, add_file,dstSubfolderSpec = "none", name = "" , dstPath = ""):
        doaddfile = []
        doaddfile.append(str(add_file))
...
class XCBuildConfiguration(PBXType):
    ...
    def change_setting_flag(self, target_key, target_value):
...
class XcodeProject(PBXDict):
    ...
    def change_setting_flag(self, target_key, target_value):
        ...
        for b in build_configs:
            if b.change_setting_flag(target_key, target_value):
                self.modified = True
    ...
    def add_copy_file(self, filename, dstFolder):

        file_refs = self.get_files_by_name(filename)

        for file_ref in file_refs:
            if isinstance(file_ref, PBXFileReference):
                parent_phase = self.get_build_phases("PBXBuildFile")
                build_file = PBXBuildFile.Create(file_ref, weak=False)
                build_file.add_attributes_flag("CodeSignOnCopy")
                build_file.add_attributes_flag("RemoveHeadersOnCopy")
                ...
                if len(targets) != 0 :
                    copyfile_phase = PBXCopyFilesBuildPhase.Create(build_file.id, "")
                    for t in targets:
                        t['buildPhases'].add(copyfile_phase.id)
                        self.objects[copyfile_phase.id] = copyfile_phase
